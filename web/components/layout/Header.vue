    ...
    <div class="relative z-10 px-4 sm:px-6 md:px-8 lg:px-12">
      <div class="flex items-center justify-between h-16 lg:h-20">
            ...
            >
              ...
              >
                ...
                <Icon 
                  name="chevron-down" 
                  size="xs" 
                  class="transition-transform duration-300 text-cyan-400 group-hover:rotate-180"
                />
              </button>
              
              <!-- Futuristic Dropdown -->
              <div 
                class="absolute top-full left-0 mt-2 w-72 bg-black/95 backdrop-blur-xl rounded-xl border border-cyan-400/30 shadow-2xl overflow-hidden transition-all duration-300 opacity-0 invisible -translate-y-2 scale-95 group-hover:opacity-100 group-hover:visible group-hover:translate-y-0 group-hover:scale-100"
                ...
                <div class="p-3 space-y-1">
                  ...
                  >
                    <div class="flex items-center justify-center w-8 h-8 bg-gradient-to-br from-cyan-400/20 to-blue-500/20 rounded-lg border border-cyan-400/30 group-hover:border-cyan-400/50 transition-all duration-300">
                      <Icon :name="getServiceIcon(child.url)" size="xs" class="text-cyan-400 group-hover:text-white transition-colors duration-300" />
                    </div>
                    <div>
                      ...
                      <div class="text-xs text-gray-400 group-hover:text-cyan-400/70 mt-1 transition-colors duration-300">
                        {{ getServiceDescription(child.url) }}
                      </div>
                    </div>
                    <!-- Hover accent -->
        ...
        <div class="flex items-center space-x-3 lg:space-x-4" ref="ctaRef">
          ...
          <div v-if="ctaButton" class="hidden md:block relative">
            <!-- Background Glow -->
            <div class="absolute inset-0 bg-gradient-to-r from-cyan-400 to-blue-500 rounded-xl opacity-75 blur-sm animate-pulse"></div>
            <NuxtLink
              :to="ctaButton.url || '/contact'"
              class="relative inline-flex items-center justify-center bg-black border-2 border-cyan-400 text-cyan-400 hover:text-black hover:bg-cyan-400 transition-all duration-300 font-mono tracking-wider uppercase text-xs lg:text-sm cyberpunk-btn px-4 py-2 lg:px-6 lg:py-3 rounded-md"
            >
              <span class="relative z-10">{{ ctaButton.text || 'Get Started' }}</span>
              <!-- Corner brackets -->
    ...
    >
      <div class="px-4 sm:px-6 md:px-8 lg:px-12 py-6">
        <nav class="space-y-1">
          <template v-for="item in navigation" :key="`mobile-${item.id}`">
            ...
            <div v-if="item.children && item.children.length > 0">
              ...
              >
                ...
                >
                  <Icon :name="getServiceIcon(child.url)" size="xs" />
          ...
          <!-- Mobile CTA -->
          <div v-if="ctaButton" class="pt-4 border-t border-cyan-400/30">
            <NuxtLink
              :to="ctaButton.url || '/contact'"
              @click="closeMobileMenu"
              class="w-full inline-flex items-center justify-center bg-black border-2 border-cyan-400 text-cyan-400 hover:text-black hover:bg-cyan-400 font-mono tracking-wider uppercase px-6 py-3 rounded-md transition-all duration-300"
            >
...
const getServiceIcon = (childUrl) => {
  const iconMap = {
    '/services/engineering': 'cpu-chip',
    '/services/seo': 'chart-bar', 
    '/services/devops': 'server',
    '/services/analytics': 'presentation-chart-line',
    '/case-studies': 'document-text',
    '/process': 'clipboard-document-list'
  }
  return iconMap[childUrl] || 'cog-6-tooth'
}

const getServiceDescription = (childUrl) => {
  const descriptions = {
    '/services/engineering': 'Custom web applications and software',
    '/services/seo': 'Search optimization and digital marketing', 
    '/services/devops': 'Cloud hosting and deployment automation',
    '/services/analytics': 'Data tracking and performance monitoring',
    '/case-studies': 'Real-world project examples',
    '/process': 'Our proven methodology'
  }
  return descriptions[childUrl] || 'Professional service'
}
