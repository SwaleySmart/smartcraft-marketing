        ...
        <div class="text-center lg:text-left space-y-8" ref="contentRef">
          ...
          >
            ...
            <BaseButton
              ...
              variant="primary"
              size="lg"
              :icon-right="ctaIcon || 'arrow-right'"
            >
              {{ ctaText }}
            </BaseButton>
...
onMounted(() => {
  nextTick(() => {
    ...
    // Enhanced glitch effect for highlight text (scoped to this component)
    const glitchElements = heroRef.value?.querySelectorAll('.hero-glitch') || []
    ...
    // Add periodic intense glitch to main highlight text (scoped to this component)
    const mainHighlight = heroRef.value?.querySelector('.hero-highlight-text')
    if (mainHighlight) {
    ...
    ScrollTrigger.create({
      ...
      onUpdate: (self) => {
        ...
        // Only animate elements that exist within this component
        const cyberGrid = heroRef.value?.querySelector('.cyber-grid-hero')
        if (cyberGrid) {
          gsap.to(cyberGrid, {
            y: progress * 50,
            duration: 0.3
        ...
        const matrixRain = heroRef.value?.querySelector('.matrix-rain')
        if (matrixRain) {
          gsap.to(matrixRain, {
            y: progress * -30,
            duration: 0.3
    ...
    // Button hover effects (scoped to this component)
    const buttons = heroRef.value?.querySelectorAll('.cyberpunk-btn-hero') || []
    buttons.forEach(btn => {
