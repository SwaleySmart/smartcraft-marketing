...
// Global SSR disabled in nuxt.config.ts

// Get current route slug
const route = useRoute()
const slug = computed(() => {
  if (!route.params.slug) {
    // Home page - no slug params
    return '/'
  }
...
// Complete GraphQL Query with blocks for dynamic pages
const GET_PAGE = gql`
...
// Execute GraphQL query (wrapped in ClientOnly to avoid SSR issues)
...
const transformedEntry = computed(() => {
  ...
  if (entry.value.blocks && Array.isArray(entry.value.blocks)) {
    transformed.blocks = entry.value.blocks.map(block => {
      ...
      delete transformedData.__typename // Remove GraphQL type from data
      ...
      if (simpleType === 'intro') {
        transformedData = {
          ...
          ctaText: transformedData.cta_text,
          ctaUrl: transformedData.cta_url,
          // Transform features array to extract color values
        ...
        // Remove original snake_case properties to avoid conflicts
...
// SEO computed properties (same logic as before)
...
const canonicalUrl = computed(() => {
  return transformedEntry.value?.canonical_url || transformedEntry.value?.url || `${useRuntimeConfig().public.siteUrl}${slug}`
})
